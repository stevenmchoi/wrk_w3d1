...
def heart_tracks
  ...
  execute(<<-SQL)
    SELECT
      ...
      COUNT(tracks.song) AS heart_count
    FROM
      tracks
    FULL JOIN
    ...
    GROUP BY
      albums.title
    ORDER BY
      heart_count DESC, album_title ASC;
  SQL
end
...
def top_track_counts
  ...
  execute(<<-SQL)
    SELECT
      albums.title, COUNT(tracks.song) AS num_tracks
    FROM
      albums
    JOIN
      tracks ON tracks.album = albums.asin
...
def rock_superstars
  ...
  execute(<<-SQL)
    SELECT
      albums.artist, COUNT(DISTINCT albums.asin) AS album_count
    FROM
      albums
    JOIN
